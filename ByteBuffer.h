...
public:
    ...
    std::vector<uint8> storage() const { return _storage; }
  ...
  /*            if(!sLog.IsOutDebug())                          // optimize disabled debug output
        ...
        sLog.outDebug(LOG_FILTER_SYSTEM, "STORAGE_SIZE: %u", size() );
        for(uint32 i = 0; i < size(); i++)
            sLog.outDebugInLine("%c", read<uint8>(i) );
    sLog.outDebug(LOG_FILTER_SYSTEM, " "); */
    }
...
protected:
    ...
    {
        std::cout << "ERROR: Attempt " << (add ? "put" : "get") << " in ByteBuffer (pos: " << pos << " size: " << size() << ") value with size: " << esize << std::endl;
...
class Pkt : public ByteBuffer
{
public:
    Pkt(size_t size) : ByteBuffer(size) { _wpos = 4; }
    Pkt(uint16 op, size_t size) : ByteBuffer(size) { _wpos = 4; SetOpcode(op); }
    Pkt() : ByteBuffer(4) { _wpos = 4; }
